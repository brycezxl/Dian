...
def train(model, train_loader, eval_loader, cfg):
    ...
    for epoch_ in range(current_epoch, cfg.NUM_EPOCHS, 1):
        running_loss = 0.0
        time_ = time()
        for i_, data_ in enumerate(tqdm(train_loader)):
            if i_ == len(train_loader) - 1:
                break
            try:
                ...
                continue
            optimizer.zero_grad()
            y_pred_raw, feature_matrix, attention_map = model(image_)
            ...
            # Attention Cropping
            with torch.no_grad():
                crop_images = batch_augment(image_, attention_map[:, :1, :, :], mode='crop', theta=(0.4, 0.6),
                                            padding_ratio=0.1)
            ...
            loss = criterion(y_pred_raw, label_) / 3. + \
                criterion(y_pred_crop, label_) / 3
            ...
            if i_ % 200 == 0:
                batch_time = time() - time_
                print("==> [train] epoch {}, batch {}, global_step {}. loss for 10 batches: {}, "
                      "time for 10 batches: {}s".format(epoch_, i_, global_step, running_loss, batch_time))
        ...
        if epoch_ % cfg.evaluate_epoch == 0:
            torch.save({
                "epoch": epoch_,
                ...
                'loss': loss,
            }, os.path.join(cfg.save_path, "train_epoch_" + str(epoch_) + ".tar"))
            print("==> [eval] on train dataset")
            acc_on_train, precision_on_train, recall_on_train = evaluate(train_loader, epoch_, model)
            print("==> [eval] on valid dataset")
            acc_on_valid, precision_on_valid, recall_on_valid = evaluate(eval_loader, epoch_, model)
